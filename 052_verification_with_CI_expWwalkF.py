# Implementation of XGBoost model verification. 
# Walk-forward sliding window for getting CI for RMSE 
#
...
def model_fit(train_X, train_y, config):
    ...
    # define model; use default objective='reg:squarederror',eval_metric='rmse',
...
def summarize_scores(filename, name, scores):
    ...
    print('%s: The error of the model was %.3f +/- %.3f at the 90%% confidence level.' % (filename, scores_m, 1.64*score_sem))
...
files = ['HPmth023.csv','HPmth044.csv', 'HPmth056.csv', 'HPmth067.csv', 'HPmth071.csv']
...
for FILE in files:
    ...
    scores = repeat_evaluate(data, config, timesteps, n_repeats)
